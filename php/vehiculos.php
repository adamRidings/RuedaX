...
try {
    ...
    $payloadAuth = null;
    $accionesProtegidas = ["crearAnuncio", "insertarImagenes", "aniadir_quitar_favorito", "obtenerFavoritos", "obtener_favorito"];

    if (in_array($action, $accionesProtegidas, true)) {
        $payloadAuth = requerir_auth($data ?? []);
    }
    ...
    if ($action === "crearAnuncio") {
        ...
        $id_usuario   = $payloadAuth['id_usuario'] ?? null;
    ...
    ";
    ...
    else if ($action === "obtenerFavoritos") {
        $id_usuario = $payloadAuth['id_usuario'] ?? null;

        if (!$id_usuario) {
            $respuesta['mensaje'] = "Falta id_usuario";
            echo json_encode($respuesta);
            exit;
        }
        ...
        FROM favoritos fav
        INNER JOIN anuncios a ON fav.id_anuncio = a.id_anuncio
        INNER JOIN vehiculos v ON a.id_vehiculo = v.id_vehiculo
        INNER JOIN usuarios u ON a.id_usuario = u.id_usuario
        LEFT JOIN fotos_vehiculos f ON f.id_vehiculo = v.id_vehiculo
        WHERE fav.id_usuario = ? AND a.estado = 'activo'
        ORDER BY a.fecha_created DESC, f.orden ASC, f.id_foto ASC
        ";

        $sentenciaGetFav = $conexion->prepare($getFavoritos);
        $sentenciaGetFav->execute([$id_usuario]);
        $rows = $sentenciaGetFav->fetchAll(PDO::FETCH_ASSOC);
    ...
    else if ($action === "aniadir_quitar_favorito") {

        $id_usuario = $payloadAuth['id_usuario'] ?? null;
        $id_anuncio = $data['id_anuncio'] ?? $_POST['id_anuncio'] ?? null;

        if (!$id_usuario || !$id_anuncio) {
            $respuesta['mensaje'] = "Faltan id_usuario o id_anuncio";
            echo json_encode($respuesta);
            exit;
        }

        $consultaFavorito = "SELECT id_favorito FROM favoritos WHERE id_usuario = ? AND id_anuncio = ? LIMIT 1";
        $sentenciaFavorito = $conexion->prepare($consultaFavorito);
        $sentenciaFavorito->execute([$id_usuario, $id_anuncio]);
        $favorito = $sentenciaFavorito->fetch(PDO::FETCH_ASSOC);

        if ($favorito) {
            $eliminar = $conexion->prepare("DELETE FROM favoritos WHERE id_favorito = ?");
            $eliminar->execute([$favorito['id_favorito']]);
            $respuesta['mensaje'] = "Favorito eliminado";
            $respuesta['favorito_activo'] = false;
        } else {
            $insertar = $conexion->prepare("INSERT INTO favoritos (id_usuario, id_anuncio) VALUES (?, ?)");
            $insertar->execute([$id_usuario, $id_anuncio]);
            $respuesta['mensaje'] = "Favorito agregado";
            $respuesta['favorito_activo'] = true;
            $respuesta['id_favorito'] = $conexion->lastInsertId();
        }

        echo json_encode($respuesta);
        exit;
    }
    else if ($action === "obtener_favorito") {
        ...
        $consulta = $conexion->prepare("SELECT id_favorito FROM favoritos WHERE id_usuario = ? AND id_anuncio = ? LIMIT 1");
        $consulta->execute([$id_usuario, $id_anuncio]);
        $favorito = $consulta->fetch(PDO::FETCH_ASSOC);

        if ($favorito) {
            $respuesta['favorito_activo'] = true;
            $respuesta['id_favorito'] = $favorito['id_favorito'];
        } else {
            $respuesta['favorito_activo'] = false;
        }
